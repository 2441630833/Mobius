...
This directory holds the **bundled Ollama runtime** and background models:

- `nomic-embed-text` — `@codebase` + skill embeddings
- `glm-ocr` — Agents image OCR preprocess

There is **no local chat model**. Agents chat uses a cloud provider from Settings / `.env`.
...
resources/ollama/
  ...
  models/        # shared model blobs (embed + OCR)
...
| Command | Purpose |
|---------|---------|
| `npm run bundle:ollama` | Download **both** amd64 + arm64 Ollama + pull `nomic-embed-text` + `glm-ocr` |
| `npm run verify:ollama` | Verify both architectures and embed/OCR models are bundled (release check) |
| `npm run setup:ollama` | Re-pull / verify embedding + OCR models |
